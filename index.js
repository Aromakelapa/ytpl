...
(async () => {
  ...
  for (let i in playlist.videos) {
    ...
    if (fs.existsSync(`downloads/${author}/${title}/${playlist.videos[i].title.replace(forbid, '-')}.mp3`)) continue;

    await download(playlist.videos[i].url, `downloads/${author}/${title}/${playlist.videos[i].title.replace(forbid, '-')}.mp3`);
  }
